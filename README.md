...
- `parse(BlossomVerb, $authHeader): Event|BlossomFailure` runs the cheap, trust-free envelope checks — header format, base64/JSON, kind, verb, `server`-tag scoping (a token naming other servers only is rejected; see [ADR-0018](docs/adr/0018-server-tag-scoping-enforced-at-parse.md)), `created_at` reasonableness (see [ADR-0016](docs/adr/0016-created-at-checked-for-reasonableness-not-strict-past.md)), and expiration — and returns the parsed (but not yet signature-verified) `Event`.
